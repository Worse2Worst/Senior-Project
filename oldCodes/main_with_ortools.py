...
def main():
    # Create the data.
    data = create_data_array('pdp_instances/LiLim/pdp_100/lc102.txt')
    locations = data[0]
    demands = data[1]
    start_times = data[2]
    end_times = data[3]
    ...
    if num_locations > 0:
        ...
        for req in requests:
            node1 = routing.IndexToNode(int(req[0]))
            node2 = routing.IndexToNode(int(req[1]))
            routing.AddPickupAndDelivery(node1,node2)
...
def create_data_array(filename):
    ...
    locations = [[v.x,v.y] for v in nodes]
    ...
    demands = [v.demand for v in nodes]

    start_times = [v.ET for v in nodes]
    end_times = [v.LT for v in nodes]
    requests = preprocessing.generate_request(nodes)
    requests = [[req[0].index,req[1].index] for req in requests]
    service_time = [v.service_time for v in nodes]
    data = [locations, demands, start_times,end_times,requests,service_time]
